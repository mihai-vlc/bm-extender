(function () {
    ...
    text = text.replace(
        /(^\[.+?\])/gm,
        '</div><div class="section"><b class="token selector block-separator italic"> $1 </b>'
    );
    ...
    text = text.replace(/((#|lineNumber: |line )\d+)/gim, '<b class="token number">$1</b>');
    text = text.replace(
        /(null|TypeError|ReferenceError| custom |importPackage|require)/gim,
        '<b class="token keyword">$1</b>'
    );
    text = text.replace(/(\w+\.(isml|jpg|png|js|ds))/gim, '<b class="token string">$1</b>');
    text = text.replace(/\[Template:([a-z0-9/]+):/gim, '[Template:<b class="token string">$1</b>:');
    text = text.replace(/\[Pipeline:([a-z]+):/gim, '[Pipeline:<b class="token string">$1</b>:');
    ...
    $el.addClass("language-log").html(
        '<code><div class="section"><h1>DW LOGS</h1>' + text + "</div></code>"
    );
